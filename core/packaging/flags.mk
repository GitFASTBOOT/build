...
# $(1): container
define _register-names-for-container
$(sort $(foreach m,$(product_MODULES),\
	$(if $(filter $(PRODUCT_OUT)/$(call _out-dir-for-partition,$(strip $(1)))/%, $(ALL_MODULES.$(m).INSTALLED)), \
		$(m)
	) \
))
endef
...
$(foreach partition, $(_FLAG_PARTITIONS), \
	$(eval aconfig_flag_summaries_protobuf.$(partition) := $(PRODUCT_OUT)/$(call _out-dir-for-partition,$(partition))/etc/aconfig_flags.pb) \
	$(eval $(call generate-partition-aconfig-flag-file, \
			$(TARGET_OUT_FLAGS)/$(partition)/aconfig_flags.pb, \
			$(aconfig_flag_summaries_protobuf.$(partition)), \
			$(partition), \
			$(sort \
				$(foreach m, $(call _register-names-for-container, $(partition)), \
...
$(foreach partition, $(_FLAG_PARTITIONS), \
	$(eval aconfig_storage_package_map.$(partition) := $(PRODUCT_OUT)/$(call _out-dir-for-partition,$(partition))/etc/aconfig/package.map) \
	$(eval aconfig_storage_flag_map.$(partition) := $(PRODUCT_OUT)/$(call _out-dir-for-partition,$(partition))/etc/aconfig/flag.map) \
	$(eval aconfig_storage_flag_val.$(partition) := $(PRODUCT_OUT)/$(call _out-dir-for-partition,$(partition))/etc/aconfig/flag.val) \
	$(eval aconfig_storage_flag_info.$(partition) := $(PRODUCT_OUT)/$(call _out-dir-for-partition,$(partition))/etc/aconfig/flag.info) \
