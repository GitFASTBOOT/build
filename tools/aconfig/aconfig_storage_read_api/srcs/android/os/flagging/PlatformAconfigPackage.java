...
public class PlatformAconfigPackage {
    ...
    private static final Set<String> PLATFORM_PACKAGE_MAP_FILE =
            Set.of(
                    MAP_PATH + "system.package.map",
                    MAP_PATH + "vendor.package.map",
                    MAP_PATH + "product.package.map");
    ...
    static {
        for (String pf : PLATFORM_PACKAGE_MAP_FILE) {
            try {
                PackageTable pTable = PackageTable.fromBytes(mapStorageFile(pf));
    ...
    /**
     * Loads an platform Aconfig Package from Aconfig Storage.
     *
