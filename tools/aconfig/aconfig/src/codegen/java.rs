...
use aconfig_protos::{FinalizedFlag, FinalizedFlags, ProtoFlagPermission, ProtoFlagState, ProtoParsedFlag};
...
fn create_flag_element(
    ...
    if finalized_sdk_present {
      println!("Finalized SDK present for flag: {}", pf.name());
    } else {
      println!("Not finalized... {}", pf.name());
    }
...
fn get_finalized_sdk_level(package: &str, pf: &ProtoParsedFlag) -> Option<i32> {
  ...
  if finalized_flags.is_err() {
    println!("Error loading finalized flags.");
    return None;
  }
  ...
  println!("No finalized flag found for flag {}", pf.name());
  None
}

fn load_finalized_flags() -> Result<FinalizedFlags> {
  // let mut f = File::open("../../data/finalized_flags_35.textproto")?;
  ...
  let mut finalized_flag = FinalizedFlag::new();
  finalized_flag.set_name("enabled_fixed_ro_exported".to_string());
  finalized_flag.set_namespace("aconfig_test".to_string());
  finalized_flag.set_package("com.android.aconfig.test".to_string());
  finalized_flag.set_container("system".to_string());
  finalized_flag.set_min_sdk(35);

  let mut finalized_flags = FinalizedFlags::new();
  finalized_flags.finalized_flag.push(finalized_flag);

  // for flag in &finalized_flags.finalized_flag {
